...
export function AdminProposalManager({ token, onPendingCountChange }: { token: string | null; onPendingCountChange?: (count: number) => void }) {
  ...
  }, [onPendingCountChange, pageNumber, query, status, token, type]);
  ...
  useEffect(() => setPageNumber(1), [query, status, type]);
  ...
  return (
    <section className="admin-list-card admin-proposal-manager">
      <div className="admin-list-card-heading">
        <div><h2>Đề xuất bổ sung thông tin KOL</h2><p>Tiếp nhận đóng góp được gửi trực tiếp từ trang thông tin KOL.</p></div>
        <span>{loading ? "Đang tải..." : `${page.total} đề xuất`}</span>
      </div>

      <div className="admin-filters">
        <label><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm KOL, email hoặc nội dung" /></label>
      ...
      <div className="admin-table-wrap">
        <table>
          <thead><tr><th>KOL</th><th>Nhóm đề xuất</th><th>Nội dung</th><th>Người gửi</th><th>Trạng thái</th><th>Ngày gửi</th></tr></thead>
          <tbody>
            {items.map((item) => (
              <tr key={item.proposal_id} onClick={() => openProposal(item.proposal_id)}>
                <td><strong>{item.influencer_name}</strong><small>{item.influencer_key}</small></td>
      ...
      {selected ? (
        <div className="admin-news-modal" role="dialog" aria-modal="true" aria-label="Chi tiết đề xuất bổ sung thông tin" onMouseDown={() => setSelected(null)}>
          <section className="admin-detail admin-proposal-detail" onMouseDown={(event) => event.stopPropagation()}>
            ...
            <div className="admin-detail-heading"><div><span className={`admin-status ${selected.status}`}>{statusLabels[selected.status]}</span><h2>{selected.influencer_name}</h2><p>{selected.proposal_type} · {new Date(selected.created_at).toLocaleString("vi-VN")}</p></div></div>
            ...
            <div className="admin-detail-grid">
              ...
              <div><strong>Mã KOL</strong><span>{selected.influencer_key}</span></div>
