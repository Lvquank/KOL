    ...
    <main className="admin-page admin-workspace">
      <section className="admin-dashboard admin-applications" data-active-tab={activeTab}>
        <header className="admin-topbar">
          ...
          <div className="admin-header-account"><span>{user.name}</span><small>{user.role === "super_admin" ? "Quản trị hệ thống" : "Chuyên viên xét duyệt"}</small><button type="button" onClick={logout}><LogOut size={16} />Đăng xuất</button></div>
        </header>
        ...
        <div className="admin-page-heading">
          <div><p>QUẢN LÝ HỒ SƠ</p><h1>Hồ sơ đăng ký</h1><small>Theo dõi, kiểm tra và xử lý các hồ sơ KOL, MCN.</small></div>
