...
export function RegistrationForm({ applicantType }: { applicantType: ApplicantType }) {
  ...
  const validOne = Boolean(form.name && form.nationality && form.address && form.email && form.zalo && (isOrganization || categoryKeys.length) && (!isOrganization || (form.businessLicenseNo && form.licenseIssuedAt && form.licenseIssuedBy && form.legalRepresentative)));
